...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            var showApduConsole by remember { mutableStateOf(false) }
            var consoleTag by remember { mutableStateOf<Tag?>(null) }

            NfcTheme {
                Surface(modifier = Modifier.fillMaxSize(), color = Color(0xFFF8FAFC)) {
                    val tagForConsole = consoleTag
                    if (showApduConsole && tagForConsole != null) {
                        ApduConsoleScreen(
                            tagProvider = { currentTag ?: tagForConsole },
                            onRefreshTag = { enableNfcReader() },
                            onBack = {
                                showApduConsole = false
                                consoleTag = null
                            }
                        )
                    } else {
    ...
    private fun Tag.readNfcData(): NfcUiState {
        ...
        val universityText = if (technologies.contains("IsoDep")) {
            exploreIsoDepCard()
        } else ""
        val canEdit = when {
            ndefInfo.isWritable -> "Sí, esta etiqueta NDEF permite escritura"
            ndefInfo.isNdef -> "No, esta etiqueta NDEF está en solo lectura"
        ...
        return NfcUiState(
            hasNfc = packageManager.hasSystemFeature(PackageManager.FEATURE_NFC),
            isNfcEnabled = nfcAdapter?.isEnabled == true,
            scanStatus = "Tarjeta leída correctamente",
            ...
            content = ndefInfo.records.ifEmpty {
                listOf("No se encontró texto o enlace NDEF público en esta tarjeta")
            },
    ...
    // ----------------------------------------------------------------------
    // FUNCIÓN PRINCIPAL DE EXPLORACIÓN ISO-DEP
    // ----------------------------------------------------------------------
    private fun Tag.exploreIsoDepCard(): String {
        val isoDep = IsoDep.get(this) ?: return "No es una tarjeta ISO-DEP"
        return try {
            isoDep.connect()
            isoDep.timeout = 3000
            val resultados = StringBuilder()

            // 1. Seleccionar Master File (MF)
            var response = isoDep.transceive(hexToBytes("00A40000023F00"))
            resultados.append("📁 SELECT MF: ${response.toHexString()} → ${response.extractText()}\n")

            // 2. Leer binario en diferentes offsets y tamaños
            val offsets = listOf(0, 16, 32, 64, 128)
            val sizes = listOf(16, 32, 64)
            for (offset in offsets) {
                for (size in sizes) {
                    val offsetHex = offset.toString(16).padStart(2, '0')
                    val sizeHex = size.toString(16).padStart(2, '0')
                    val cmd = hexToBytes("00B000$offsetHex$sizeHex")
                    response = isoDep.transceive(cmd)
                    if (response.endsWithSuccessStatus() && response.size > 2) {
                        val data = response.copyOfRange(0, response.size - 2)
                        resultados.append("📖 READ offset $offset, size $size: ${data.toHexString()} → ${data.extractText()}\n")
                    }
                }
            }

            // 3. GET DATA (información del archivo actual)
            response = isoDep.transceive(hexToBytes("80CA000000"))
            if (response.endsWithSuccessStatus()) {
                resultados.append("📄 GET DATA: ${response.toHexString()} → ${response.extractText()}\n")
            }

            // 4. Probar selección de DF por AIDs comunes
            val aids = listOf(
                "A0000000041010",       // Visa
                "A000000003000000",     // MasterCard
                "D2760000850101",       // NDEF Type 4
                "315041592E535953",     // 1PAY.SYS (transporte)
                "A000000018434D",       // American Express
                "D2760000050101",       // Generic
                "A0000000040001"        // Mifare
            )
            for (aid in aids) {
                val aidLength = (aid.length / 2).toString(16).padStart(2, '0')
                val cmd = hexToBytes("00A40400$aidLength$aid")
                response = isoDep.transceive(cmd)
                if (response.endsWithSuccessStatus()) {
                    resultados.append("🎯 AID encontrado: $aid → ${response.extractText()}\n")
                    // Intentar leer después de seleccionar el AID
                    val readCmd = hexToBytes("00B0000010")
                    val readResp = isoDep.transceive(readCmd)
                    if (readResp.endsWithSuccessStatus()) {
                        resultados.append("   ➜ Datos AID: ${readResp.extractText()}\n")
                    }
                }
            }

            // 5. Comando extra: obtener UID del tag (a veces funciona)
            response = isoDep.transceive(hexToBytes("FFCA000000")) // get UID command (algunas cards)
            if (response.endsWithSuccessStatus()) {
                resultados.append("🆔 UID interno: ${response.extractText()}\n")
            }

            if (resultados.isEmpty()) "No se encontró ningún dato legible"
            else resultados.toString()

        } catch (e: Exception) {
            "Error en exploración: ${e.message}"
        } finally {
            try { isoDep.close() } catch (_: Exception) {}
        }
    }

    // Función auxiliar: extrae texto ASCII si es legible, o muestra HEX
    private fun ByteArray.extractText(): String {
        val ascii = this.toPrintableText().replace(".", "").trim()
        return if (ascii.isNotEmpty() && ascii.length > 2) {
            "📝 ASCII: $ascii"
        } else {
    ...
    // ----------------------------------------------------------------------
    // RESTO DE FUNCIONES AUXILIARES (las que ya tenías)
    // ----------------------------------------------------------------------
    ...
    private fun NfcReaderScreen(state: NfcUiState, onOpenApduConsole: () -> Unit) {
        val statusText = when {
            ...
            !state.isNfcEnabled -> "Activa NFC en ajustes del teléfono"
        ...
        LazyColumn(
            ...
            // Mostrar contenido universitario (resultados de exploración)
            if (state.universityText.isNotBlank() &&
                !state.universityText.startsWith("No se encontró") &&
                !state.universityText.startsWith("Error")) {
                item {
                    Card(
                        ...
                        Column(modifier = Modifier.padding(16.dp)) {
                            Text(
                                text = "🔍 Datos extraídos de la tarjeta",
                                fontWeight = FontWeight.Bold,
            ...
            item { SectionTitle("Contenido NDEF") }
            if (state.content.isEmpty()) {
                item { SmallCard("No hay contenido NDEF público.") }
            } else {
                items(state.content) { SmallCard(it) }
            }

            if (state.technologies.isNotEmpty()) {
                item { SectionTitle("Tecnologías detectadas") }
                items(state.technologies) { SmallCard(it) }
            }

            if (state.technicalData.isNotEmpty()) {
                item { SectionTitle("Datos técnicos") }
        ...
        NfcTheme {
            NfcReaderScreen(
                state = NfcUiState(
                    ...
                    tagType = "ISO-DEP / tarjeta inteligente",
                    canEdit = "No detectable",
                    maxSize = 0,
                    technologies = listOf("IsoDep", "NfcB"),
                    content = listOf("No hay NDEF"),
                    technicalData = listOf("ATQA: ..."),
                    universityText = "SELECT MF → HEX: 6A 82\nREAD offset 0 → ASCII: HolaMundo"
                ),
                onOpenApduConsole = {}
            )
        }
